...
const UnitLessonPage = () => {
  ...
  const handleSpeakSlow = async (word: WordData) => {
    if (isSilent()) return;
    setSpeaking(true);
    try {
  ...
  const handleNext = () => {
    ...
    if (currentIndex < lesson.words.length - 1) {
      ...
      navigate(`/unit/${unitId}/lesson/${lessonId}/practice`);
    }
  };
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => handleSpeakSlow(word)}
                disabled={speaking}
                className="w-12 h-12 rounded-full flex items-center justify-center bg-orange-100 dark:bg-orange-900/20 hover:bg-orange-200 dark:hover:bg-orange-900/30 transition-colors"
                title="Lassított tempó"
              >
            ...
            {word.sentences?.[0] && (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-lg text-foreground leading-relaxed">{word.sentences[0].en}</p>
                </div>
                <p className="text-base text-muted-foreground">{word.sentences[0].hu}</p>
              </button>
            )}
          </motion.div>
        </AnimatePresence>
